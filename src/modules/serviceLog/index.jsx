...
function VehicleMo({initial,customers,onSave,onClose}){
  ...
  return(<Mo title={initial?"Edit Equipment":"Add Equipment"} onClose={onClose} onSave={()=>{if(!f.name.trim())return alert("Name required.");onSave(f);}} saveLabel={initial?"Save Changes":"Add Equipment"} large>
    ...
    {f.maintReminders.length>0&&<div className="reminder-entry" style={{fontSize:"10px",color:"var(--text-dim)",letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:"3px"}}><span>Reminder</span><span>Every (hrs)</span><span>Or every (mo)</span><span></span></div>}
    {f.maintReminders.map((r,i)=>(<div key={r.id||i} className="reminder-entry"><Fi placeholder="e.g. Oil Change" value={r.label} onChange={e=>updReminder(i,"label",e.target.value)}/><Fi type="number" placeholder="250" value={r.intervalHours} onChange={e=>updReminder(i,"intervalHours",e.target.value)}/><Fi type="number" placeholder="6" value={r.intervalMonths} onChange={e=>updReminder(i,"intervalMonths",e.target.value)}/><button className="btn btn-danger btn-xs" onClick={()=>remReminder(i)}>✕</button></div>))}
