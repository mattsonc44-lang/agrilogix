...
export default function App() {
  ...
  const [module,     setModule]     = useState(()=>localStorage.getItem('al_active_module')||null);
  ...
  useEffect(() => {
    if (!loading && session && profile?.tenantId && !module) {
      ...
      if (available.length > 0) {
        const saved = localStorage.getItem('al_active_module');
        setModule(saved && available.includes(saved) ? saved : available[0]);
      }
    }
  ...
  const loadUserProfile = async (sess) => {
    ...
    try {
      ...
      if (userProfile?.tenantId) {
        ...
        if (mods.length) {
        const saved = localStorage.getItem('al_active_module');
        setModule(saved && mods.includes(saved) ? saved : mods[0]);
      }
      } else {
      ...
      <div style={{ background:T.brand, padding:"0 16px", display:"flex", alignItems:"center", gap:"0", position:"sticky", top:0, zIndex:50 }}>
        ...
        <div style={{ display:"flex", flex:1, overflowX:"auto" }}>
          {enabledModules.map(mid => {
            ...
            return (
              <button key={mid} onClick={()=>{ setModule(mid); localStorage.setItem('al_active_module', mid); }} style={{
